      ...
      db.query('SELECT * FROM users WHERE email_user = $1', [email_user], (err, email) => {
        ...
        db.query('SELECT * FROM users WHERE username = $1', [username], (err, user) => {
          ...
          bcrypt.hash(password, saltRounds, async function (err, hash) {
            ...
            const { rows } = await db.query('WITH inserted AS (INSERT INTO users(email_user,username,password,role,id_region) VALUES ($1, $2, $3, $4, $5) RETURNING *) SELECT * FROM inserted LEFT JOIN regions ON inserted.id_region = regions.id_region;', [email_user, username, hash, role, id_region])
